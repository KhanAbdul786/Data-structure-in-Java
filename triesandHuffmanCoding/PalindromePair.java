...
public class PalindromePair {
	...
	private void print(TrieNode4 root, String word) {
		if (root == null) {
			return;
		}
		...
		for (TrieNode4 child : root.children) {
			if (child == null) {
				continue;
			}
	...
	private boolean isPalindromePair(TrieNode4 root, ArrayList<String> words) {
	...
	private boolean doesPairExistFor(TrieNode4 root, String word, int startIndex) {
		...
		if (startIndex == word.length()) {
			if (root.isTerminating) {
				/*
				 * When there exists an exact match of the word we are searching for in the
				 * Trie, irrespective of the trie extends further in the same path.
				 *
				 */
				return true;
			}
			/*
			 * When trie extends further, there doesnt exist an exact match of the string we
			 * are looking for. However, it may happen that any of the the branch extending
			 * from root may form a palindrome.
			 *
			 */
		...
		TrieNode4 correspondingChild = root.children[charIndex];
		if (correspondingChild == null) {
			/*
			 * This means that, down the line in the characters that we are looking for
			 * against the word in the trie, the character doesn't exist in the trie. This
			 * character can be anywhere in the range [0 - (word length-1)]
			 *
			 */
			if (root.isTerminating) {
	...
	private boolean checkRemainingBranchesInTrie(TrieNode4 root, String word) {
		/*
		 *
		 * This function recursively explores all the branches from the root while
		 * keeping a track of the characters in the 'word' and checks each word whenever
		 * the terminating condition is true.
		 *
		 * If the word is a palindrome, we return true and stops exploring the other
		 * branches. If it is otherwise, we keep on checking the remaining branches.
		 *
		 * Once all the branches are explored, and we don't find any branch or word that
		...
		for (TrieNode4 childNode : root.children) {
			if (childNode == null) {
				continue;
			}
